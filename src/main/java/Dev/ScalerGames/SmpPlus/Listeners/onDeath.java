    ...
    public void onDeath(PlayerDeathEvent event) {
        ...
        if (Main.getInstance().getConfig().getBoolean("Events.Death.enabled")) {
            ...
            event.setDeathMessage(Format.placeholder(event.getEntity().getPlayer(), Main.getInstance().getConfig().getString("Events.Death.message")
                    .replace("{player}", event.getEntity().getPlayer().getName()).replace("{message}", event.getDeathMessage())
                    .replace("{displayname}", event.getEntity().getPlayer().getDisplayName()).replace("{world}", event.getEntity().getPlayer().getWorld().toString())));
